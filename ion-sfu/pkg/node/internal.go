...
func (s *server) Publish(stream pb.SFU_PublishServer) error {
	...
	for {
		...
		if err != nil {
			...
			log.Logger.Errorf("publish error %v %v", errStatus.Message(), errStatus.Code())
			return err
		}
		...
		case *pb.PublishRequest_Connect:
			...
			log.Logger.Infof("publish->connect called: %v", payload.Connect)

			pub, reply, err = s.publish(payload)

			if err != nil {
				log.Logger.Errorf("publish->connect: error publishing stream: %v", err)
				pub.Close()
				return err
			}
...
func (s *server) Subscribe(stream pb.SFU_SubscribeServer) error {
	...
	for {
		...
		if err != nil {
			...
			log.Logger.Errorf("subscribe error %v", err)
			return err
		}
		...
		case *pb.SubscribeRequest_Connect:
			...
			if err != nil {
				log.Logger.Errorf("subscribe->connect: error subscribing stream: %v", err)
				return err
			}
			...
			if err != nil {
				log.Logger.Errorf("subscribe->connect: error subscribing to stream: %v", err)
				sub.Close()
				return nil
			}
