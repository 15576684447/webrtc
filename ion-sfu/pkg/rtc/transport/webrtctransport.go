...
func NewWebRTCTransport(id string, options RTCOptions) *WebRTCTransport {
	...
	w.pc.OnICECandidate(func(c *webrtc.ICECandidate) {
		...
		if remoteSDP == nil {
			...
			log.Logger.Infof("w.pc.OnICECandidate remoteSDP == nil c=%v", c)
		} else {
			log.Logger.Infof("w.pc.OnICECandidate remoteSDP != nil c=%v", c)
			w.candidateCh <- c
		}
	...
	w.pc.OnICEConnectionStateChange(func(connectionState webrtc.ICEConnectionState) {
		...
		case webrtc.ICEConnectionStateDisconnected:
			log.Logger.Infof("webrtc ice disconnected for mid: %s", id)
		case webrtc.ICEConnectionStateFailed:
			log.Logger.Infof("webrtc ice failed for mid: %s", id)
			w.Close()
		case webrtc.ICEConnectionStateClosed:
			log.Logger.Infof("webrtc ice closed for mid: %s", id)
			w.Close()
		}
	})
...
func (w *WebRTCTransport) Answer(offer webrtc.SessionDescription, options RTCOptions) (webrtc.SessionDescription, error) {
	...
	if w.isPub {
		...
		//添加sub时，根据pub的tracks，生成sub的ssrcpt
		ssrcPTMap := options.Ssrcpt
		if len(ssrcPTMap) == 0 {
			return webrtc.SessionDescription{}, errInvalidOptions
		}
		//如果是sub端，每连接一个sub端，就增加pub端的tracks到sub端的pc，用于发送数据到sub端
			...
			if _, found := w.outTracks[ssrc]; !found {
				...
				if track != nil {
					...
					if err == nil {
						...
						log.Logger.Errorf("w.pc.AddTrack err=%v", err)
					}
				}
			}
		}
		//为每个sub端的连接创建一个sctp的读取任务，存放到rtcpChan
	...
	if err != nil {
		log.Logger.Errorf("pc.SetLocalDescription answer=%v err=%v", answer, err)
	}
	go func() {
		w.candidateLock.Lock()
		defer w.candidateLock.Unlock()
		for _, candidate := range w.pendingCandidates {
			log.Logger.Infof("WebRTCTransport.Answer candidate=%v", candidate)
...
func (w *WebRTCTransport) receiveInTrackRTP(remoteTrack *webrtc.Track) {
	for {
		...
		if err != nil {
			...
			log.Logger.Errorf("rtp err => %v", err)
		}
		w.rtpCh <- rtp
	}
}
...
func (w *WebRTCTransport) WriteRTP(pkt *rtp.Packet) error {
	...
	//发送RTP时，如果待发送RTP包的PayloadType与该pc支持的PayloadType不同，则转换该RTP包的PayloadType再发送
	if srcType != destType {
		...
		if candid, ok := ptTransformMap[srcType]; ok {
			...
			for _, k := range candid {
				if destType == k {
					// Do the transform
					newPkt := *pkt
					log.Logger.Tracef("Transforming %v => %v", srcType, destType)
	...
	if track == nil {
		log.Logger.Errorf("WebRTCTransport.WriteRTP track==nil pkt.SSRC=%d PT=%d", pkt.SSRC, pkt.Header.PayloadType)
		return errInvalidTrack
	}

	log.Logger.Tracef("WebRTCTransport.WriteRTP pkt=%v", pkt)
	err := track.WriteRTP(pkt)
	if err != nil {
		log.Logger.Errorf("WebRTCTransport.WriteRTP => %s", err.Error())
		w.writeErrCnt++
		return err
	}
	return nil
...
func (w *WebRTCTransport) Close() {
	...
	w.stop = true
	log.Logger.Infof("WebRTCTransport.Close t.ID()=%v", w.ID())
...
func (w *WebRTCTransport) receiveOutTrackRTCP(sender *webrtc.RTPSender) {
	for {
		...
		if err != nil {
			log.Logger.Errorf("rtcp err => %v", err)
		}
