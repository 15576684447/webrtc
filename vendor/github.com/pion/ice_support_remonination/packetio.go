...
var ErrFull = errors.New("ice.Buffer is full, discarding write")
...
func (b *Buffer) Write(origPacket []byte) (n int, err error) {
	// Copy the packet before adding it.
	// packet = append([]byte{}, packet...)
	packet := make([]byte, len(origPacket))
	copy(packet, origPacket)
...
// WriteWithoutMakeAndCopy has similar function with Write, but has no Make and Copy
func (b *Buffer) WriteWithoutMakeAndCopy(packet []byte) (n int, err error) {
	b.mutex.Lock()
...
// ReadWithoutCopy has similar function with Read, but has no Copy
func (b *Buffer) ReadWithoutCopy() (packet []byte, err error) {
	for {
		b.mutex.Lock()
