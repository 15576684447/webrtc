...
func (a *Agent) gatherCandidatesLocal(networkTypes []NetworkType) {
	...
	var localIPs []net.IP
	var localGenerations []int

	if len(a.ips) != 0 {
		for ip, gen := range a.ips {
			localIPs = append(localIPs, net.ParseIP(ip))
			localGenerations = append(localGenerations, gen)
		}
	} else {
		ips, err := a.localInterfaces(networkTypes)
		if err != nil {
			a.log.Warnf("failed to iterate local interfaces, host candidates will not be gathered %s", err)
			return
		}
		localIPs = ips
		localGenerations = make([]int, len(ips))
	}

	wg.Add(len(localIPs) * len(supportedNetworks))
	for idx, ip := range localIPs {
		mappedIP := ip
		generation := localGenerations[idx]
		...
		for _, network := range supportedNetworks {
			go func(network string, ip, mappedIP net.IP, generation int) {
				defer wg.Done()
				if len(a.udpGodPorts) != 0 {
					gathered := 0
					for _, godport := range a.udpGodPorts {
						//reuse port connection
						localAddr := mappedIP.String()
						c, err := NewCandidateReuse(&CandidateHostConfig{
							Network:    network,
							Address:    localAddr,
							Port:       int(godport),
						...
						if err != nil {
							a.log.Warnf("Failed to create reuse candidate: %s %s %d: %v\n", network, mappedIP, godport, err)
							continue
						}
						c.currAgent = a
						d := getDispatcher(localAddr, godport)
						err = d.RegisterCand(godport, localAddr, c, nil, "", "")
						if err != nil {
							a.log.Warnf("Failed to create reuse candidate: %s %s %d: %v\n", network, mappedIP, godport, err)
						} else {
							c.conn = d.GetRandomConn(localAddr + ":" + strconv.Itoa((int)(godport)))
			...
			}(network, ip, mappedIP, generation)
		}
	}
}
